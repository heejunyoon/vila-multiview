...
class Coord3DPatchEmbed(nn.Module):
    def __init__(self, embed_dim=1152, hidden_dim=512, patch_shape=(27, 27), depth=4):
        ...
        self.mlp = ResidualMLP(3, hidden_dim, embed_dim, depth)
        # self.mlp = nn.Sequential( #단순한 버전
...
class SiglipEncoder(nn.Module):
    ...
    def __init__(self, config: SiglipConfig):
        super().__init__()
        self.config = config
        self.layers = nn.ModuleList([SiglipEncoderLayer(config) for _ in range(config.num_hidden_layers)]) #설정한 개수만큼 실행
    ...
    def forward(
        ...
        pixel_values (`torch.FloatTensor` of shape `(batch_size, num_channels, height, width)`):
            ...
            [`AutoImageProcessor`]. See [`CLIPImageProcessor.__call__`] for details.
        return_loss (`bool`, *optional*):
            Whether or not to rnputs_embeds
        for encoder_layer in self.layers: # vision encoder 각 레이어 갖고와서 진행
            if output_hidden_states:
                encoder_states = encoder_states + (hidden_states,)
            if self.gradient_checkpointing and self.training: # 학습 진행할 경우
            ...
            else: # inference 진행하는 경우.
                # print("llava>model>multimodal_encoder>siglip>modeling_siglip.py>line 1024>hidden_states")
                layer_outputs = encoder_layer(
                    hidden_states,
                    attention_mask,
                    output_attentions=output_attentions,
                )
            # print(f"llava>model>multimodal_encoder>siglip>modeling_siglip.py>line 1028>layer_outputs: {layer_outputs[0].shape}")
